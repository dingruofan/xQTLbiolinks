...
#' @import stringr
#' @importFrom GenomicRanges GRanges
#' @importFrom GenomicFeatures intronicParts exonParts
...
#' }
# xQTLanalyze_anno <- function(snpInfo="", genomeVersion="hg38", pValueBy=5){
#
...
#     snpInfoNew <- data.table::data.table(chrom = as.character(GenomeInfoDb::seqnames(snpRanges_hg38)), pos=as.data.table(IRanges::ranges(snpRanges_hg38))$start, pValue=snpRanges_hg38$pValue)
...
#
#   ############## import datasets:
#   message("==> Start import annotations...")
#   message("      cpg...")
#   cpgAnno <- fread(system.file(package = "xQTLbiolinks", "extdata", "cpgLsland.bed.gz"), header=FALSE)
#   cpgRanges <- GenomicRanges::GRanges(cpgAnno$V1, IRanges::IRanges(cpgAnno$V2, cpgAnno$V3), strand= "*")
#
#   # import enhancer dataset:
#   message("      enhancer...")
#   enhancerAnno <- fread(system.file(package = "xQTLbiolinks", "extdata", "geneHancer.bed.gz"), header=FALSE)
#   enhancerRanges <- GenomicRanges::GRanges(enhancerAnno$V1, IRanges::IRanges(enhancerAnno$V2, enhancerAnno$V3), strand= "*")
#
...
#   tfRanges <- GenomicRanges::GRanges(tfAnno$V1,IRanges::IRanges(tfAnno$V2, tfAnno$V3), strand= "*")
#
#   # extract introns:
#   message("      introns...")
...
#   message("==> Start annotate variants...")
#   cpgHits <- as.data.table(SummarizedExperiment::findOverlaps(query = snpRanges, subject = cpgRanges, maxgap = 1, ignore.strand=TRUE ))
...
#   enhancerHits <- as.data.table(SummarizedExperiment::findOverlaps(query = snpRanges, subject = enhancerRanges, maxgap = 1, ignore.strand=TRUE ))
...
#   tfHits <- as.data.table(SummarizedExperiment::findOverlaps(query = snpRanges, subject = tfRanges, maxgap = 1, ignore.strand=TRUE ))
...
#   intronHits <- as.data.table(SummarizedExperiment::findOverlaps(query = snpRanges, subject = intronParts, maxgap = 1, ignore.strand=TRUE ))
...
#   rm(intronParts, intronAnno, convertCharacter)
#
#   # annotate snp with exons:
#   exonHits <- as.data.table(SummarizedExperiment::findOverlaps(query = snpRanges, subject = exonParts, maxgap = 1, ignore.strand=TRUE ))
...
#   promoterHits <- as.data.table(SummarizedExperiment::findOverlaps(query = snpRanges, subject = promoterParts, maxgap = 1, ignore.strand=TRUE ))
...
#   cdsHits <- suppressWarnings( as.data.table(VariantAnnotation::locateVariants(snpRanges, txdb, VariantAnnotation::CodingVariants())) )
#   if(nrow(cdsHits)>0){
...
#   utr5Hits <- suppressWarnings( as.data.table(VariantAnnotation::locateVariants(snpRanges, txdb, VariantAnnotation::FiveUTRVariants())) )
#   if(nrow(utr5Hits)>0){
...
#   utr3Hits <- suppressWarnings( as.data.table(VariantAnnotation::locateVariants(snpRanges, txdb, VariantAnnotation::ThreeUTRVariants())) )
#   if(nrow(utr3Hits)>0){
...
#   splicingHits <- suppressWarnings( as.data.table(VariantAnnotation::locateVariants(snpRanges, txdb, VariantAnnotation::SpliceSiteVariants())) )
#   if(nrow(splicingHits)>0){
#     splicingHits <- cbind(snpInfo[splicingHits$QUERYID,], splicingHits[,.(tx_id=as.integer(TXID),type="splicsSite")])
...
#
#   intergenicHits <- suppressWarnings( as.data.table(VariantAnnotation::locateVariants(snpRanges, txdb, VariantAnnotation::IntergenicVariants(), ignore.strand=TRUE)) )
...
#     intergenicHits[,c("anno", "type"):=.(paste0(unlist(lapply(intergenicHits$PRECEDEID, fetchLastElement)),"-",unlist(lapply(intergenicHits$FOLLOWID, fetchFirstElement))), "ingergenic")]
...
#     )
#   snpHitsCount <- snpHits[,.(Num=nrow(.SD)), by= c("cutP", "type")]
#   snpHitsCount[Num==1, "Num"] <- 2
#   ggplot(snpHitsCount, aes(x = cutP ,y = Num,fill=type)) +
...
#   )+
#   xlab("ATC")+
#   ylab("mean num of ADRs")	#坐标轴翻转
#
#
#   snpHitsCount2 <- snpHits[]
# }
#

#
# a_680Ranges <- GenomicRanges::GRanges(a_680$seqnames,IRanges::IRanges(a_680$start, a_680$end), strand= "*")
# a_680alls <- VariantAnnotation::locateVariants(a_680Ranges, txdb, VariantAnnotation::AllVariants(), ignore.strand=TRUE)
# a_680intron <- locateVariants(a_680Ranges, txdb, IntronVariants()) %>% as.data.table
# a_680out <- IRanges::findOverlaps(a_680Ranges, intronicParts(txdb) )
#
#
# ####
# intronParts <-  intronicParts(txdb)
# exonParts <- exonicParts(txdb)
# FiveUTRVariants
# ThreeUTRVariants
# PromoterVariants(upstream=1000, downstream=0)
# SpliceSiteVariants
#
# tf <- fread("D:\\R_project\\Data\\Annotation\\tf_filtered_sorted_merge_sub.bed")
# tfRanges <- GenomicRanges::GRanges(tf$V1,IRanges::IRanges(tf$V2, tf$V3), strand= "*")
# geneHancer <- fread("D:\\R_project\\Data\\Annotation\\geneHancer.txt")
# enhancerRanges <- GenomicRanges::GRanges(geneHancer$chrom, IRanges::IRanges(geneHancer$chromStart, geneHancer$chromEnd), strand= "*")
# cpg <- fread("D:\\R_project\\Data\\Annotation\\cpgLsland.txt")
# cpgRanges <- GenomicRanges::GRanges(cpg$chrom, IRanges::IRanges(cpg$chromStart, cpg$chromEnd), strand= "*")
#
# IRanges::findOverlaps(a_680Ranges, tfRanges )
#
# # enhancer > promoter > cds > exon > splice > 5UTR > 3UTR > intron > intergenics
# # ignore.strand
# ####
#
# IRanges::findOverlaps(snpRanges,tfRanges )
# IRanges::findOverlaps(snpRanges,enhancerRanges )
# IRanges::findOverlaps(snpRanges,cpgRanges )
#
# library(TxDb.Hsapiens.UCSC.hg38.knownGene )
# txdb <- TxDb.Hsapiens.UCSC.hg38.knownGene
# alls <- VariantAnnotation::locateVariants(snpRanges, txdb, VariantAnnotation::AllVariants())
#
# #
# #
# # ##
# # 38:
# load("D:/R_project/Data/GCST006085/envColocBoth_h38.RData")
# snpRanges <- GenomicRanges::GRanges(ifelse(is.na(sentinelSnpDF$chr),"NA",sentinelSnpDF$chr),
#                                     IRanges::IRanges(ifelse(is.na(sentinelSnpDF$position),-1,sentinelSnpDF$position), ifelse(is.na(sentinelSnpDF$position),-1,sentinelSnpDF$position)),
#                                     strand= "*"
...
#
# aa<- unique(aa[,.(seqnames, start,end,LOCATION)])
#
#
# aaa <- merge( unique(aa[duplicated(aa[,.(seqnames,start,end)]),.(seqnames, start, end)]), aa, by=c("seqnames", "start", "end"))
#
#
# enhancerRanges <- GenomicRanges::GRanges(ifelse(is.na(geneHancer$chrom),"NA",geneHancer$chrom),
#                                          IRanges::IRanges(ifelse(is.na(geneHancer$chromStart),-1,geneHancer$chromStart), ifelse(is.na(geneHancer$chromEnd),-1,geneHancer$chromEnd)),
#                                          strand= "*"
# )
#
# tfRanges <- GenomicRanges::GRanges(tf$V1,IRanges::IRanges(tf$V2, tf$V3), strand= "*")
#
# IRanges::intersect(snpRanges,enhancerRanges)
