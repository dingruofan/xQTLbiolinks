...
retrieveLD_LDproxy <- function(snp, pop = "CEU", r2d = "r2", token = "9246d2db7917",windowSize=500000, file = FALSE){
  ...
  if (!((grepl(rsid_pattern, snp, ignore.case = TRUE)) | (grepl(chr_coord_pattern,
                                                                snp, ignore.case = TRUE)))) {
    stop("Invalid query format for variant: ", snp,
         ".", sep = "")
  }
  ...
  body <- list(paste("var=", snp, sep = ""),
               ...
               paste("window",windowSize, sep=""))
  url_str <- paste(url, "?", paste(unlist(body), collapse = "&"),
                   sep = "")
