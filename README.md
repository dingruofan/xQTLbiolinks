...
1.  `xQTLbiolinks` has been successfully installed on Mac OS X, Linux, and Windows, using the `install.packages` function to install directly from CRAN with command `install.package("xQTLbiolinks")`. We recommend installing the latest version from GitHub (`remotes::install_github("dingruofan/xQTLbiolinks")`) to access more advanced software features. Check more details in section "Installation" below.
