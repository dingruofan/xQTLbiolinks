...
eqtlAsso <- GTExdownload_assoAll(traitGene, tissueSiteDetail = tissueSiteDetail)
# Keep three columns for following analyses:
eqtlAsso <- eqtlAsso[,.(snpId, pValue, maf)] 
eqtlAsso
```
...
```{r}
gwas_eqtl <- merge(gwasDF[,.(snpId=rsid, pValue=P, maf=maf, position)], eqtlAsso, by="snpId", sort=FALSE, suffixes = c(".gwas",".eqtl"))
...
```{r fig.width=6.1, fig.height=4.7, message=FALSE}
GTExvisual_locusCompare( sentinelSnps[,.(rsid=snpId, pval=pValue.eqtl)], sentinelSnps[,.(rsid=snpId, pval=pValue.gwas)] )
```
...
# if(requireNamespace(locuscomparer)){
