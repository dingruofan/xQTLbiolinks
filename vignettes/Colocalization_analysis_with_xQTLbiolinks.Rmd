...
colocResultsigGene <- colocResultsigGene[geneType =="protein coding"]
```
...
```{r results = 'hide', echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
xQTLvisual_genesExp(colocResultsigGene$geneSymbol, tissueSiteDetail=tissueSiteDetail)
```
...
For the potential cancer-related trait gene `MMP7` that with the highest `PPH4.ABF=0.9978`, expression of which in multiple tissues can be plotted with `xQTLvisual_geneExpTissues`:
```{r results = 'hide', echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
...
To gain insight into the function of trait genes with high PPH4, and explore the potential regulatory mechanism of the prostate cancer, we first download expression profiles of the genes with higher value of PPH4 (>0.75) in prostate.

```{r results = 'hide', echo=TRUE, message=FALSE, warning=FALSE, eval=FALSE}
expMat <- xQTLdownload_exp(colocResultsigGene$gencodeId, tissueSiteDetail=tissueSiteDetail, toSummarizedExperiment =FALSE)
```
...
For convenience, we convert `expMat` to a data.table object and retain gene name as a column.
```{r eval=TRUE}
expMat <- na.omit( cbind(data.table( gencodeId = rownames(expMat) ), as.data.table(expMat) ) )
expMat[1:5,1:4]
```
