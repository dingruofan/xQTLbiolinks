...
4.Conduct colocalization analysis. Following three steps of colocalization analysis are encapsulated in one function xQTLanalyze_coloc:
  ...
  Outputs is a data.table that combined all results of `coloc_Out_summary` of all genes. The posterior probability of variants fo each hypothesis (H0-H4) are listed in `colocResultAll`:
  ```{r eval=TRUE}
  head(colocResultAll)
  ```

5.We considered coloc tests with a Posterior probability of hypothesis 4 (PPH4.ABF) ≥ 0.75 as having strong or moderate evidence for colocalization.
  ...
  colocResultsig
  ```
6.To gain insight into the function of these genes, and explore the potential regulatory mechanism of the prostate cancer, we first fetched genes' details with `xQTLquery_gene`:

  ```{r eval=TRUE, message=FALSE}
  colocResultsigGene <- xQTLquery_gene(colocResultsig$traitGene)
  ```
  Add PPH4 for each gene:
  ```{r eval=TRUE}
  ...
  colocResultsigGene
  ```
  Co-expression analysis can be conducted among these genes:
  ```
  exp <- xQTLdownload_exp(colocResultsigGene$geneSymbol, tissueSiteDetail=tissueSiteDetail,toSummarizedExperiment=FALSE)
  ```
